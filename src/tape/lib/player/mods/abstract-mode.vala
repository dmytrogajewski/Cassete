...
public abstract class Tape.PlayerMode : Object {
    ...
    public virtual async void send_play_async (string play_id,
        ...
        if (get_current_track_info () == null) {
            debug ("[PLAYER] send_play_async: current_track_info is null, returning");
            return;
        }

        debug ("[PLAYER] send_play_async: Calling form_play_obj()");
        var play_obj = form_play_obj ();
        debug ("[PLAYER] send_play_async: form_play_obj returned, context=%s, context_item=%s",
               play_obj.context, play_obj.context_item ?? "(null)");
        ...
        debug ("[PLAYER] send_play_async: Calling yam_helper.send_play()");
        try {
            yield root.yam_helper.send_play ({ play_obj });
            debug ("[PLAYER] send_play_async: yam_helper.send_play() completed");
        } catch (Tape.CantUseError e) {
            warning ("Failed to send play feedback: %s", e.message);
        }
    }
