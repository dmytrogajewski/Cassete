...
public class Tape.Storager : Object {
    ...
    File _audios_playbackdir_file;
    public File audios_playbackdir_file {
        get {
            if (_audios_playbackdir_file == null) {
                _audios_playbackdir_file = File.new_build_filename (
                    audios_cachedir_file.peek_path (),
                    "playback"
                );
            }
            create_dir_if_not_existing (_audios_playbackdir_file);

            return _audios_playbackdir_file;
        }
    }
    ...
    File get_audio_playback_file (string track_id) {
        return File.new_build_filename (
            audios_playbackdir_file.peek_path (),
    ...
    public async void save_audio (
        ...
        try {
            ...
            // Invalidate decoded playback copy if it exists
            var playback_file = get_audio_playback_file (track_id);
            if (playback_file.query_exists ()) {
                try {
                    playback_file.delete ();
                } catch (Error e) {
                    warning ("Can't remove decoded playback copy for %s: %s", track_id, e.message);
                }
            }
        } catch (Error e) {
    ...
    public async string? ensure_audio_playback_file (string track_id) {
        Location audio_location = audio_cache_location (track_id);
        ...
        try {
            if (playback_file.query_exists ()) {
                var encoder_info = audio_location.file.query_info (
                    "time::modified,standard::size",
                    FileQueryInfoFlags.NONE,
                    null
                );
                var playback_info = playback_file.query_info (
                    "time::modified,standard::size",
                    FileQueryInfoFlags.NONE,
                    null
                );

                if (playback_info.get_size () > 0 &&
                    playback_info.get_attribute_uint64 ("time::modified") >=
                    encoder_info.get_attribute_uint64 ("time::modified")) {
                    need_update = false;
                }
            }
        } catch (Error e) {
        ...
        if (need_update) {
            uint8[]? audio_data = yield load_audio_data (track_id);

            if (audio_data == null || audio_data.length == 0) {
                return null;
            }
        ...
        return playback_file.peek_path ();
    }
